...
Reverse-engineered from the official Allo Plus app v2.4.77.10. Full RE
notes in the private companion repo (see README).

Flow:
  1. POST https://intelbras-4.qvcloud.net:443/auth/user;jus_duplex=down
     (bootstrap, gets initial cookies)
  2. POST .../jus_duplex=up with login envelope. Receives jsessionid cookie.
  3. POST .../jus_duplex=down again to read login result (envelope-on-wire).
  4. POST .../jus_duplex=up with get-device-list envelope.
  5. POST .../jus_duplex=down to read out-auth-code (OAC) for the monitor.
  6. POST http(s)://<monitor-ip>/tdkcgi with set.device.opendoor envelope
     containing sha256(UNLOCK_PIN) in the body.

After step 5 the OAC is cached on disk for 12h (configurable); subsequent
unlocks skip the cloud entirely.
"""
...
def _ssl_no_verify() -> ssl.SSLContext:
    """The Qualvision cloud uses a self-signed `eziotest` cert and pins
    everything client-side. We do the same as the SDK: TLS yes, verify no."""
...
def _encode_device_password(pwd: str) -> str:
    """Replicates `QvEncrypt.EncodeDevicePassword` from the Quvii SDK.

    If the input is already 64 hex chars (i.e. an OAC), passes through;
    otherwise applies SHA-256 hex. This is what the device expects in the
    `<password>` field of the body envelope for both check_password and
    opendoor commands.
    """
    if len(pwd) >= 64:
        return pwd
...
class AlloWT7Client:
    ...
    async def get_device_status(
        ...
        *,
        ...
        """Poll the LAN device status. Returns a dict with at least 'calling' (bool).

        Uses get.device.status via /tdkcgi. The <calling> field transitions to
        'true' when the doorbell button is pressed, giving real-time detection
        without any cloud dependency or FCM token.
        """
        body = _build_lan_envelope(oac, LAN_CMD_DEVICE_STATUS)
        schemes = [scheme] if scheme == "https" else ["http", "https"]
        for sch in schemes:
                    ...
                    async with session.post(
                        ...
                        if err != LAN_ERROR_OK:
                            if err == LAN_ERROR_AUTH_INVALID:
                                raise AlloWT7AuthError("OAC expired (get_device_status)")
                            raise AlloWT7Error(f"get_device_status error={err!r}")
                        ds = root.find(".//devicestatus")
                        calling_txt = ds.findtext("calling", "false") if ds is not None else "false"
                        return {
                            "calling": calling_txt.lower() == "true",
                            "lockstatus": (ds.findtext("lockstatus", "false").lower() == "true") if ds is not None else False,
                        }
                except aiohttp.ClientConnectorError:
                    continue
        raise AlloWT7ConnectionError(f"monitor unreachable at {monitor_ip} (get_device_status)")
